...
class MODEL(nn.Module):
    def __init__(self, res=False, cont=False):
        ...
        nhid = 20
        self.cell = HessianDampingNODE(tempf(nhid, nhid), corr=0, corrf=False)
...
def main():
    model = MODEL()
    trainpv(model, 'output/pv_log_hd0_{}.csv'.format(count_parameters(model)), 'output/pv_hdnode_rnn.mdl')
